...
public class KafkaResetter extends AbstractResetter implements ConsumerSeekAware {
    ...
    public static final String RESET_IDENTIFIER_FORMAT = "format";
    ...
    public void handleDataInfo(
        ...
        try {
            ...
            if (valueSet.contains(RESET_IDENTIFIER_FORMAT)) {
                try {
                    LOGGER.info("接收到格式化重置消息, 正在重置格式化...");
                    context.resetFormat();
                } catch (Exception e) {
                    LOGGER.warn("重置格式化时发生异常, 格式化将不会重置, 异常信息如下: ", e);
                }
            }
            ack.acknowledge();
        } finally {
