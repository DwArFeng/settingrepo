...
public class SupportCommand extends CliCommand {
    ...
    private static final String COMMAND_OPTION_RESET_FORMATTER = "reset-formatter";

    private static final String[] COMMAND_OPTION_ARRAY = new String[]{
            COMMAND_OPTION_RESET_FORMATTER
    };

    private static final String IDENTITY = "support";
    private static final String DESCRIPTION = "支持操作";

    private static final String CMD_LINE_SYNTAX_RESET_FORMATTER = IDENTITY + " " +
            CommandUtil.concatOptionPrefix(COMMAND_OPTION_RESET_FORMATTER);

    private static final String[] CMD_LINE_ARRAY = new String[]{
            CMD_LINE_SYNTAX_RESET_FORMATTER
    };
    ...
    protected List<Option> buildOptions() {
        ...
        list.add(Option.builder().longOpt(COMMAND_OPTION_RESET_FORMATTER).desc("重置格式化器支持").build());
        return list;
    }
    ...
    protected void executeWithCmd(Context context, CommandLine cmd) throws TelqosException {
        try {
            ...
            switch (pair.getLeft()) {
                case COMMAND_OPTION_RESET_FORMATTER:
                    supportQosService.resetFormatter();
                    context.sendMessage("重置格式化器支持成功。");
                    break;
            }
        } catch (Exception e) {
